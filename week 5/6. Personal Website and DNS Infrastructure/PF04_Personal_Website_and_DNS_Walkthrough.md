...
- 🌐 **Primary Vercel Production URL**: [https://abdulhayykhan-portfolio.vercel.app](https://abdulhayykhan-portfolio.vercel.app)
...
- 📂 **Portfolio Codebase Path**: `Portfolio/` (Next.js 15, React 19, TypeScript, Tailwind CSS)
...
|                                                                                       |
|  User types: "abdulhayykhan-portfolio.vercel.app"                                     |
|         |                                                                             |
|         v                                                                             |
...
|         |                                                                             |
|         v                                                                             |
|  [ 4. TLD Nameserver (.app) ]       ---> Points to Vercel Authoritative Server.        |
|         |                                                                             |
|         v                                                                             |
|  [ 5. Authoritative Nameserver ]    ---> Looks up CNAME/A record -> Returns Edge IP   |
|         |                                                                             |
|         v                                                                             |
...
Think of **DNS as the internet’s universal phonebook**. Computers communicate across the global network using numerical IP addresses (like `76.76.21.21`). Humans, however, remember names like `abdulhayykhan-portfolio.vercel.app`. DNS automatically translates human-readable web addresses into machine-readable numerical IP addresses in milliseconds.

### 2. What Happens Between Typing a URL and Seeing the Page?

When someone types `abdulhayykhan-portfolio.vercel.app` into their browser, five steps happen under the hood:
...
4. **TLD Nameserver Lookup**: The **.app TLD Nameserver** directs the resolver to **Vercel’s Authoritative Nameserver** (`ns1.vercel-dns.com`).
...
Every file deployed in the `Portfolio/` codebase serves a specific structural, styling, or routing purpose:

| Deployed File Path | File Purpose & Technical Function |
|---|---|
| `Portfolio/app/page.tsx` | Main Next.js App Router entry page rendering hero section, project grid, and profile links. |
| `Portfolio/app/layout.tsx` | Root HTML layout component establishing font imports, metadata, and dark mode theme wrappers. |
| `Portfolio/app/globals.css` | Global CSS styling rules and Tailwind utility directives. |
| `Portfolio/public/Abdul_Hayy_Khan_Resume.pdf` | Authentic CV document served directly over static asset route (`/Abdul_Hayy_Khan_Resume.pdf`). |
| `Portfolio/public/icon.svg` | Vector brand icon for site header and browser favicon. |
| `Portfolio/next.config.ts` | Next.js build configuration rules for static asset optimization and header caching. |
